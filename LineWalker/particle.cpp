	...
	for (int i = 0; i < particles; i++)
	{
		...
		switch (m_shape)
		{
			case Shape::CIRCLE:
			{
				...
				bool cosNeg = (cosAngle < 0.00f);
				bool sinNeg = (sinAngle < 0.00f);
				UniRealDist randomAngleCos(0.0f, abs(cosAngle));
				UniRealDist randomAngleSin(0.0f, abs(sinAngle));
				particle->vel.x = (cosNeg) ? -randomAngleCos(gen) : randomAngleCos(gen);
				particle->vel.y = (sinNeg) ? -randomAngleSin(gen) : randomAngleSin(gen);
